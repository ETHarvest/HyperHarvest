...
export async function agent(context: HandlerContext) {
...
function generateSystemPrompt(context: HandlerContext) {
  ...
  const systemPrompt = `You are a helpful agent that lives inside a web3 messaging one to one app.\n
  These is the user : ${JSON.stringify(members?.map((member: User) => ({ ...member, username: `@${member.username}` })))}\n 
  This user app has many commands available: ${JSON.stringify(commands)}\n
